(function (streaming, global) {
    global.Backbone.sync = function (method, model, options) {
        var message = {
            method: method,
            url: model.url(),
            cid: model.cid,
        ...
        streaming.socket.emit("sync", message, function (err, response) {
            if (err) {
                options.error(err);
            } else {
    ...
    streaming.setup = function setup(socket) {
    ...
    streaming.emit = function emit(url, item) {
        ...
        streaming.socket.emit("stream", message, function (err, response) {
            if (!err) {
                item.cache = response;
    ...
    streaming.connect = function (socket) {
        streaming.socket = socket;
    ...
    streaming.add = function (target, options) {
        var url = target.url();
        if (target.streaming) {
            return;
        }
    ...
    streaming.remove = function (target) {
        if (target.streaming) {
            ...
            if (_.keys(item.clients).length == 0) {
                item.timeout = setTimeout(function () {
                    streaming.socket.emit("unstream", { url: url }, function (err, response) {});
    ...
    _.extend(global.Backbone.Model.prototype, {
        stream: function () {
    ...
    _.extend(global.Backbone.Collection.prototype, {
